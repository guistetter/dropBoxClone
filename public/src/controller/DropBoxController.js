class DropBoxController{
    ...
    constructor(){
        ...
        this.currentFolder = ['hcode'];//criar pastas ['hcode', 'pc', 'arquivos', 'imgs']

        this.onselectionchange = new Event('selectionchange');

        this.navEl = document.querySelector('#browse-location');//breadcrumbs
    ...
    var config = {
        ...
        apiKey: "AIzaSyADJedcOFoP9zSwjSaiHpHBtUkvaJVQaWs",
        authDomain: "dropbox-clone-785bf.firebaseapp.com",
        databaseURL: "https://dropbox-clone-785bf.firebaseio.com",
        projectId: "dropbox-clone-785bf",
        storageBucket: "dropbox-clone-785bf.appspot.com",
        messagingSenderId: "85791316323",
        appId: "1:85791316323:web:b0836637829cd098ec1b56",
        measurementId: "G-CW3NZH1CZR"
        };//fim config
    ...
    removeTask(){
        ...
        this.getSelection().forEach(li =>{
            ...
            /* refatorado para remover do firebase storage e database
            let formData = new FormData();

            formData.append('path', file.path);
            formData.append('key', key);
            promises.push(this.ajax('/file','DELETE', formData));
            //return Promise.all(promises);o return aqui esta dentro do => entao retorna pra funcao foreach e nao pro removeTask
            */
    ...
    uploadComplete(){
        ...
        this.inputFilesEl.value = '';//zera o campo importante para zerar o campo de nome de arquivo e tal..
        this.btnSendFileEL.disabled = false;
    }

    getFirebaseRef(path){//pegar referencia para salvar

        if(!path) path = this.currentFolder.join('/'); //join metodo para array 
    ...
    uploadTask(files){//posso subir um arquivo ou N, mudar no index.ejs o input file para multiple
  
            //colocando dados no storage do firebase
        let promises = [];

        [...files].forEach(file => {      //files é uma coleção converter para array com o spread
           ...
           /* 
            let formData = new FormData();//estamos tentando enviar arquivo, precisamos por dentro do forma data
            
            formData.append('input-file', file)//campo q o post recebae o arquivo q esta sendo enviado
    ...
    uploadProgress(event, file){
        ...
        this.timeleftBarEl.innerHTML = this.formatTimeToHuman(timeleft);
    ...
    formatTimeToHuman(duration){
        ...
        if(hours > 0){
            ...
            return `${hours} horas, ${minutes} minutos e ${seconds} seconds`;
        }
    ...
    /*getFileView(file, key){ //funcao para inserir icone conforme o tipo de arquivo, pasta, texto, imagem..
    ...
    getFileView(file, key){
        ...
        <div class="name text-center">${file.name}</div>
        `
        this.initEventsLi(li);//pegar o arquivo selecionado e dar destaque
        return li;
    ...
    readFiles(){
        ...
        this.getFirebaseRef().on('value', snapshot =>{
            ...
            snapshot.forEach(snapshotItem =>{
                ...
                if(data.type){//validacao só renderiza na tela se tiver um type....
    ...
    /* readFiles(){//select dos dados com firebase
        //on "ouve" como se fosse um evento startando 
        this.getFirebaseRef().on('value', snapshot => {
            this.listFilesEL.innerHTML = '';//zeramos 
            
            snapshot.forEach(snapshotItem =>{
                let key = snapshotItem.key;
                let data = snapshotItem.val();
                console.log(key, data);
                this.listFilesEL.appendChild(this.getFileView(data, key)); //listamos os novos
    ...
    renderNav(){ //bread crumb
        ...
        for (let i=0; i < this.currentFolder.length; i++){
            ...
            path.push(folderName);
                ...
                if((i + 1) === this.currentFolder.length){//se a posicao atual mais é igual o length já estou na ultima posicao
    ...
    initEventsLi(li){//selecionar arquivo clicado

        li.addEventListener('dblclick', e =>{//navegar entre pastas 
        ...
        li.addEventListener('click', e=>{
            ...
            if(e.shiftKey){
                ...
                if(firstLi){ //selecionar com shift 
                    let indexStart;
                    let indexEnd;
                    ...
                    let index = [indexStart, indexEnd].sort();//ordenar
